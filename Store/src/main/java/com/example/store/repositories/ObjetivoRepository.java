...
public interface ObjetivoRepository extends JpaRepository<ObjetivoModelo, Long>{
    @Transactional
    @Modifying
    @Query(value = "INSERT INTO objetivos (id_obj, nombre, valor, descripcion,categoria) VALUES (?1, ?2, ?3, ?4,?5)", nativeQuery = true)
    int insertar(Long id, String nombre, Long valor, String descripcion,String categoria);
    
    
    @Transactional
    @Modifying
    @Query(value = "UPDATE objetivos SET nombre = ?2 WHERE id_obj = ?1", nativeQuery = true)
    int updateNombre(Long id_usuario,String nuevo);
    
    @Transactional
    @Modifying
    @Query(value = "UPDATE objetivos SET valor = ?2 WHERE id_obj = ?1", nativeQuery = true)
    int updateValor(Long id_usuario,Long nuevo);
    
    @Transactional
    @Modifying
    @Query(value = "UPDATE objetivos SET descripcion = ?2 WHERE id_obj = ?1", nativeQuery = true)
    int updateDescripcion(Long id_usuario,String nuevo);
    
    @Transactional
    @Modifying
    @Query(value = "UPDATE objetivos SET categoria = ?2 WHERE id_obj = ?1", nativeQuery = true)
