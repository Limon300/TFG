...
import pruebagrafica.datos.SensorResponse;
    ...
    public int[] actualizaVision(){
        ...
        for(int i = 0; i < vision.length;i++){
            ...
            if(!sensor.getCasillas().isEmpty()){
                System.out.println(i + " z: " + sensor.getCasillas().getFirst().getZ() + " y a: " + z);
                if((mov >= sensor.getCasillas().getFirst().getZ() - z) && (mov >= z - sensor.getCasillas().getFirst().getZ())){
